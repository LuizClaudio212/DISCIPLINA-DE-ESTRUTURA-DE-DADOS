...
def test_remove_non_existing_element():
    ...
    set_queue.add(1)
    try:
        set_queue.remove(2)  # Deve lançar ElementNotFound
        assert False, "Erro: Exceção não lançada para elemento não existente"
    except nao_existe as e:
        assert str(e) == "Erro! O elemento não está na fila.", "Erro: Mensagem de erro inesperada"
